...
float3 randomShift(float3 vec, float3 normal, float radius, unsigned global_id, unsigned useed) {
    ...
    float3 shiftVec;
    shiftVec.x = u01fixedpt_closed_closed_32_float(u.i.x);
    shiftVec.y = u01fixedpt_closed_closed_32_float(u.i.y);
    shiftVec.z = u01fixedpt_closed_closed_32_float(u.i.z);
...
kernel void emitRays(global Ray *rayList, int width, int height, int nRaysPerPixel,
    ...
    for (int i = 0; i < nRaysPerPixel; i++) {
        ...
        float3 shiftedViewPoint = randomShift(viewPoint, viewVector, 0.04f, global_id, seed);
